...
GEN encodeinput(GEN n, GEN t, long long int input){
    if(input>itos(t)){
        printf("[Error] Using the current encoding and t value, this input will give incorrect result due to reduction\n");
        return stoi(0);
    }
    GEN ret;
...
GEN encrypt(GEN input, params* param, keyset* key){
    if(lg(input)-1 >= itos(param->n)){
        printf("The input vector cannot be greater than n \n");
        return stoi(0);
    }
    GEN n = param->n;
    GEN q = param->q;
...
GEN decryptSimple(GEN input, params* param, keyset* key){
    ...
    printf("Simple decryption result %s\n", GENtostr(result));
